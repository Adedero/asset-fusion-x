...
export const investmentPlanSchema = z
  .object({
    name: z
      .string("Invalid investment plan name")
      .nonempty("Investment plan name is required"),
    category: z.enum(
      [
        "forex",
        "stocks",
    ...
    minimumDeposit: z
      .number("Invalid minimum deposit")
      .gt(0, "Minimum deposit must be greater than 0"),
    maximumDeposit: z
      .number("Invalid maximum deposit")
      .gt(0, "Maximum deposit must be greater than 0"),
    duration: z
      .number("Invalid duration")
      .gte(1, "Duration must be at least 1 day"),
    profitDistribution: z.enum(
      ["daily", "weekly", "bi_weekly", "monthly"],
      "Invalid profit distribution"
    ),
    percentageTotalReturn: z
      .number("Invalid total return percentage")
      .gt(100, "Total return percentage must be greater than 100%"),
    percentagePeriodicReturn: z.number(),
    terminationFee: z
      .number("Invalid termination fee")
      .gte(0, "Termination fee must be at least 0")
  })
  ...
  .refine(
    (data) => {
      ...
      const divisor = divisorMap[data.profitDistribution] ?? 1;
      return data.duration % divisor === 0;
    },
    {
      error: "Duration must align with profit distribution cycle",
      path: ["duration"] // show error on duration field
    }
  );
